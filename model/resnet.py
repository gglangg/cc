...
class ResNet_Bottleneck_OS16(nn.Module):
    def __init__(self, num_layers):
        ...
        self.layer5 = make_layer(Bottleneck, in_channels=4*256, channels=128, num_blocks=3, stride=1, dilation=2)
...
class ResNet_BasicBlock_OS16(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 18:
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("C:/Users/hsiuter/MedicalImageAnalysis/Project_2/MedicalImage_Project02_Segmentation/model/resnet18-5c106cde.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
